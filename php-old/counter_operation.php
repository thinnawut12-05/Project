    ...
    try {
        switch ($action) {
            case 'walk_in_check_in':
                ...
                // ************************************************************************
                // แก้ไขตรงนี้: ลบ Phone_Number ออกจาก INSERT statement
                ...
                // สำหรับ walk-in, Email_member อาจเป็น generic email หรือ NULL
                $generic_email_member = "walkin@example.com"; // หรือจะปล่อยเป็น NULL ก็ได้ ขึ้นอยู่กับ DB schema
                $num_rooms = 1; // สำหรับ Walk-in ทีละห้อง

                // แก้ไขตรงนี้: ลบ 's' (สำหรับ Phone_Number) ออกจาก bind_param และลบ $contact_phone ออกจากพารามิเตอร์
                $stmt_insert_reservation->bind_param(
                    "ssiiisssisd", // ลบ 's' สุดท้ายออก
