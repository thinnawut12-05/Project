...
$status_id_late_checkin_adjusted = 8; // *** เพิ่ม: สถานะเช็คอินล่าช้า/ถูกปรับ (สมมติ ID เป็น 8) ***
        ...
        try {
            switch ($action) {
                ...
                case 'check_out_booking':
                    ...
                    // *** แก้ไข: อัปเดตสถานะการจองในตาราง 'reservation' เป็น 'เสร็จสมบูรณ์' (Booking_status_Id = 7)
                    // เพิ่มสถานะที่อนุญาตให้เช็คเอาท์ได้ (Checked-in หรือ Late Check-in/Adjusted)
                    $valid_pre_checkout_status_ids = [
                        $status_id_checked_in,
                        $status_id_late_checkin_adjusted // อนุญาตให้เช็คเอาท์จากสถานะนี้
                    ];
                    $pre_checkout_placeholders = implode(',', array_fill(0, count($valid_pre_checkout_status_ids), '?'));
                    $pre_checkout_types = str_repeat('i', count($valid_pre_checkout_status_ids));

                    $stmt_update_reservation_status = $conn->prepare(
                        "UPDATE reservation SET Booking_status_Id = ?
                        WHERE Reservation_Id = ? AND Province_Id = ? AND Booking_status_Id IN ($pre_checkout_placeholders)"
                    );
                    ...
                    $bind_params_checkout_update = array_merge(
                        [$status_id_completed, $reservation_id, $current_province_id],
                        $valid_pre_checkout_status_ids
                    );
                    $bind_types_checkout_update = 'isi' . $pre_checkout_types;

                    call_user_func_array([$stmt_update_reservation_status, 'bind_param'], array_merge([$bind_types_checkout_update], $bind_params_checkout_update));

                    $stmt_update_reservation_status->execute();
                    if ($stmt_update_reservation_status->affected_rows === 0) {
                        throw new Exception("ไม่พบการจอง หรือสถานะไม่ถูกต้องสำหรับเช็คเอาท์ (ต้องเป็นสถานะ 'เช็คอินแล้ว' หรือ 'เช็คอินล่าช้า/ถูกปรับ').");
                    }
                ...
                case 'cancel_booking':
                    $valid_pre_cancel_status_ids = [
                        ...
                        $status_id_checked_in,
                        $status_id_late_checkin_adjusted // สามารถยกเลิกสถานะนี้ได้เช่นกัน
                    ];
                    ...
                    $cancel_target_status_id = $status_id_cancelled_timeout; // สามารถเปลี่ยนเป็น status_id_cancelled_by_officer หากมี
...
// เพิ่มเงื่อนไขเพื่อไม่แสดงสถานะ "เสร็จสมบูรณ์" (เช็คเอาท์แล้ว) ตามค่าเริ่มต้นของการดึงข้อมูล
// หากต้องการให้แสดงเมื่อเลือก filter "ทั้งหมด" หรือ "เช็คเอาท์แล้ว" ก็จะถูก override ด้วย filter ด้านบน
else {
    $sql_bookings .= " AND r.Booking_status_Id != ?";
    $params[] = $status_id_completed;
    $param_types .= 'i';
}
...
<head>
    ...
    <style>
        ...
        */
        ...
        /* Secondary gray */
        /* *** เพิ่ม: สไตล์สำหรับสถานะ "เช็คอินล่าช้า/ถูกปรับ" *** */
        .status-เช็คอินล่าช้า-ถูกปรับ {
...
<body>
    ...
    <main class="container">
        ...
        <?php if (empty($bookings)): ?>
            <p style="text-align: center; padding: 20px; font-size: 1.1em; color: #666;">ไม่พบข้อมูลการจองณตอนนี้!!.</p>
        <?php else: ?>
                ...
                <table class="booking-table">
                    <thead>
                        <tr>
                            <th>รหัสการจอง</th>
                            <th>ชื่อลูกค้า</th>
                            <!-- <th>ประเภทห้องที่จอง</th> --> <!-- ลบคอลัมน์นี้ออก -->
                            <th>จำนวนห้อง</th>
                            <th>ผู้ใหญ่/เด็ก</th>
                            <th>เช็คอิน</th>
                            <th>เช็คเอาท์</th>
                    ...
                    <tbody>
                        <?php foreach ($bookings as $booking):
                            // *** เพิ่มเงื่อนไขนี้เพื่อไม่แสดงข้อมูลที่สถานะเป็น "เสร็จสมบูรณ์" (เช็คเอาท์แล้ว) ***
                            if ($booking['Booking_status_Id'] == $status_id_completed) {
                                continue; // ข้ามการแสดงผลแถวนี้
                            }
                        ...
                        ?>
                            <tr>
                                ...
                                <!-- <td><?= htmlspecialchars($booking['Room_types_booked'] ?? 'ไม่ระบุ') ?></td> --> <!-- ลบคอลัมน์นี้ออก -->
                                ...
                                <td class="action-buttons-cell">
                                    ...
                                    <?php
                                    // *** แก้ไข: ปุ่มเช็คเอาท์จะแสดงเมื่อสถานะเป็น "เช็คอินแล้ว" หรือ "เช็คอินล่าช้า/ถูกปรับ" ***
                                    $can_check_out = ($current_status_id == $status_id_checked_in || $current_status_id == $status_id_late_checkin_adjusted);
                                    ?>
    ...
    <div id="bookingDetailsModal" class="modal">
        <div class="modal-content">
            ...
            <p><b>ประเภทห้องที่จอง:</b> <span id="modalRoomTypesBooked"></span></p> <!-- ยังคงแสดงใน modal -->
    ...
    <script>
        ...
        function openBookingDetailsModal(button) {
            ...
            document.getElementById('modalRoomTypesBooked').textContent = booking.Room_types_booked ? booking.Room_types_booked : 'ไม่ระบุ'; // แสดงประเภทห้องใน modal
